...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 16 16:11:51 2022
...
def plot_ensemble_correlation(ens_corr_sim,ens_corr_sim_place,
    ...
    pv_mouse=np.loadtxt(
        "/home/federico/postdoc/Data from Zivlab 2/Data_Liron/drive-download-20210723T135015Z-001/ens_correlation_AB_all_maps_sem_active.dat",delimiter=',')
    
    pv_mouse_place=np.loadtxt('/home/federico/postdoc/Data from Zivlab 2/Data_Liron/drive-download-20210723T135015Z-001/ens_correlation_AB_all_maps_sem_place.dat',delimiter=',')
   ...
   # ax[0].plot(np.arange(0,ndays),ens_corr_sim_place_old,'-*',color='orange',markersize=ms2,
    ...
    ax[1].set_ylabel('Ensemble corr.')
    ax[1].set_xlabel(r'$\Delta t$ (sessions)',labelpad=padx)
    
    ax[1].set_ylim([np.min(ens_corr_sim)-0.1,1.1])
    ax[1].set_yticks([np.round(np.min(ens_corr_sim),decimals=1),1])
    ax[1].set_yticklabels([np.round(np.min(ens_corr_sim),decimals=1),1])
    ax[1].set_xticks([0,7])
